...
export function Economies() {
  ...
  const hasOverlapRisk = data.wasteful.byPoste.length > 0 && data.subscriptionsToCancel.length > 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-lg bg-white p-4 shadow-sm ring-1 ring-slate-200">
        ...
        {hasOverlapRisk && (
          <p className="mt-2 text-xs text-slate-400">
            Un abonnement également marqué comme dépense inutile peut être compté dans les deux catégories
      ...
      <section className="rounded-lg bg-white p-4 shadow-sm ring-1 ring-slate-200">
        ...
        {data.wasteful.byPoste.length === 0 ? (
          <p className="mt-2 text-sm text-slate-500">Aucune dépense inutile identifiée pour cette année.</p>
        ) : (
          <ul className="mt-2">
            {data.wasteful.byPoste.map((item) => (
